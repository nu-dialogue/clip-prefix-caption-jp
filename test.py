...
from model import ClipPrefixModel
DEVICE = torch.device("cuda:0") if torch.cuda.is_available() else torch.device("cpu")

class Predictor:
    def __init__(self, model: ClipPrefixModel) -> None:
    ...
    def generate_beam(self, embed, beam_size, prompt=None, entry_length=67, temperature=1.0):
        ...
        return output_texts
    def __init__(self, clip_model, clip_preprocesser, model) -> None:
        self.clip_model = clip_model
        self.preprocess = clip_preprocesser
        self.tokenizer = T5Tokenizer.from_pretrained("rinna/japanese-gpt2-medium")
        self.model = model
    ...
    def beam_generate(self, embed, beam_size, prompt=None, entry_length=67, temperature=1.0):
        stop_token_index = self.tokenizer.encode(self.stop_token)[0]
        tokens = None
        scores = None
        device = next(self.model.parameters()).device
